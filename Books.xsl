...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <table id="menuTable" border="1" class="indent">
      ...
      <tbody id="sectionFiction">
        <xsl:call-template name="loopFiction">
          <xsl:with-param name="size" select="number(2)" />
  ...
  <xsl:template name="loopFiction">
    ...
    <xsl:choose>
      <xsl:when test="$totalNumOfEntreeSize &gt; 0">

        <!-- 
          if $size is somewhat set manually by user and is > $totalNumOfEntreeSize by accident, 
          initialize $size = $totalNumOfEntreeSize
          so that $loadSetSize(a total # of loadSet) at later step can be a value of 1
          
          This doesn't guarantee such a good UI representation when a DB to load is huge enough
          Hoewever, this can work as an additional filter when unexpected change by user occurred        
        -->
        <xsl:if test="$size &gt; $totalNumOfEntreeSize">
          <xsl:call-template name="loopFiction">
            <xsl:with-param name="size" select="$totalNumOfEntreeSize" />
          </xsl:call-template>
        </xsl:if>
      </xsl:when>
      <xsl:otherwise>
        <p>
          no element exists in the section
          <b>Fiction</b>
        </p>
        <p>please check xml file</p>
      </xsl:otherwise>
    </xsl:choose>
